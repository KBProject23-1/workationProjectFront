  ...
  >
    ...
    >
      <header class="flex items-center justify-between px-5 pt-5 pb-3">
        <h2 id="reference-sheet-title" class="text-title font-bold text-ink">
          기준 장소 선택
        </h2>
        <button type="button" aria-label="닫기" class="text-ink-mute" @click="$emit('close')">
          <X :size="20" />
        </button>
      </header>

      <div class="px-5 pb-3">
        <label class="flex h-11 items-center gap-2 rounded-chip border border-line px-3">
          <Search :size="16" class="shrink-0 text-ink-mute" />
          <input
            v-model="keyword"
            type="search"
            placeholder="장소 이름 검색"
            aria-label="장소 이름 검색"
            class="w-full min-w-0 border-0 p-0 text-body outline-none placeholder:text-ink-mute"
          />
        </label>
      </div>

      <div class="min-h-0 flex-1 overflow-y-auto px-5 pb-6">
        <p v-if="isSearching" class="py-8 text-center text-body-sm text-ink-mute">
          찾는 중...
        </p>
        ...
        <template v-else>
          <button
            type="button"
            class="flex w-full items-center justify-between gap-3 border-b border-line py-3 text-left"
            @click="$emit('select', null)"
          >
            <span class="min-w-0 flex-1">
              <span class="block truncate text-body font-bold text-ink">
                이 지역 전체
              </span>
          ...
          >
            ...
            <span
              v-if="place.reserved"
              class="shrink-0 rounded-full bg-brand-weak px-2 py-0.5 text-caption font-bold text-brand"
            >
              예약함
            </span>
            <Check
          ...
          <p
            ...
            class="py-10 text-center text-body-sm text-ink-mute"
          >
            검색 결과가 없어요
          </p>
        </template>
