<template>
  <div class="rounded-2xl bg-blue-50 px-4 py-4">
    <div class="mb-2 flex items-start justify-between">
      <span
        class="rounded-md bg-white px-2 py-1 text-[11px] font-semibold text-blue-600"
      >
        진행 중
      </span>
      <span
    ...
    <div class="mt-2 flex justify-between text-xs text-slate-500">
      <span
        >전체 {{ workation.totalDays }}일 ·
        {{ workation.elapsedDays }}일째</span
      >
      <span>{{ remainDays }}일 남음</span>
    </div>
  </div>
</template>
...
const remainDays = computed(
  () => props.workation.totalDays - props.workation.elapsedDays,
);
</script>
