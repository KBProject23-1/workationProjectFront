...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    /**
     * 이메일 인증번호 발송 — POST /users/me/email/verification 호출
     * - 이메일 변경 전, 변경할 새 이메일(email)로 인증번호를 발송한다 (Mock 발송 — 서버가 임시 저장)
     * - 인증번호 원문은 이 스토어에 저장하지 않는다 (응답에도 포함되지 않는다 — 서버만 보관)
     * - 성공 data.email(정규화된 발송 이메일)을 반환해 화면이 확인 단계에서 동일 이메일을 전달하도록 한다
     * - 실패 시 err 를 그대로 throw → 화면에서 useErrorToast 로 안내한다.
     */
    async sendEmailVerification(email) {
      ...
      this.error = null;
      try {
        const { data } = await sendEmailVerificationApi({ email });
        return data;
      } catch (err) {
        this.error = err;
        throw err;
    ...
    /**
     * 이메일 인증번호 확인 — POST /users/me/email/verification/confirm 호출
     * - 발송된 인증번호(verificationCode)가 올바른지 확인한다 — 검증은 전부 서버가 수행한다
     *   (프론트에서 인증번호를 직접 비교하지 않는다 — 서버 응답으로만 성공을 판단)
     * - 성공 시 서버가 해당 이메일을 인증 완료 상태(VERIFIED)로 저장하고 data.verified=true 를 반환한다
     *   (실제 이메일 변경은 별도 changeEmail API 에서 처리)
     * - 인증번호 원문은 이 스토어에 저장하지 않는다 (Request Body 로만 전달)
     * - 실패 시 err 를 그대로 throw → 화면에서 useErrorToast 로 안내한다.
     */
    async confirmEmailVerification({ email, verificationCode }) {
      this.isLoading = true;
      this.error = null;
      try {
        const { data } = await confirmEmailVerificationApi({
          email,
          verificationCode,
        });
        return data;
    ...
    /**
     * 이메일 변경 — PATCH /users/me/email 호출
     * - Request Body 를 받지 않는다 — 인증번호 확인(confirm)으로 인증 완료된 이메일을 서버가
     *   조회해 변경한다 (프론트가 이메일을 다시 전송하지 않는다 — 프론트 전달 이메일 신뢰 금지)
     * - 성공 응답 data.updatedEmail 을 user.email 에 반영해 내 정보 화면이 최신 이메일을 표시한다
     *   (변경 후 확인 버튼에서 fetchMyInfo 로도 한 번 더 갱신한다)
     * - 실패 시 err 를 그대로 throw → 화면에서 useErrorToast 로 안내한다.
     */
    async changeEmail() {
      ...
      this.error = null;
      try {
        const { data } = await changeEmailApi();
        this.user = {
          ...this.user,
    ...
    /**
     * 회원 탈퇴 — DELETE /users/me 호출 후 인메모리 인증 상태를 정리한다.
     * - 현재 비밀번호(password)를 전달해 본인 인증(민감 작업 추가 인증 — knowledge.md)을 수행한다.
     * - 탈퇴 성공 시 백엔드가 Access/Refresh Token Cookie 를 즉시 만료(Max-Age=0) 처리하므로
     *   프론트에서 Token 을 직접 삭제하지 않는다 (HttpOnly Cookie — JavaScript 접근 금지).
     * - isAuthenticated/user/accountVerified/sessionChecked 를 초기화한다 (탈퇴 후 로그인 화면 이동 전제).
     * - 이 기기 PIN 등록 캐시(pinRegistry) 초기화는 화면에서 처리한다 (LoginPage/PinSetupPage 와 동일 패턴).
     * - 비밀번호 원문은 이 스토어에 저장하지 않는다 (Request Body 로만 전달).
     * - 실패 시 err 를 그대로 throw → 화면에서 useErrorToast 로 안내한다.
     */
    async withdraw(password) {
      ...
      this.error = null;
      try {
        await withdrawAccountApi(password);
        this.isAuthenticated = false;
        this.user = null;
