  ...
  <div class="min-h-screen bg-white px-5 pt-4 pb-8">
    ...
    <template v-else-if="current">
      ...
      <div class="mt-4 grid grid-cols-2 gap-3">
        ...
        >
          <img :src="expenseIcon" alt="" class="h-[22px] w-[22px]" />
          지출
        </button>
        <button
          class="flex flex-col items-center gap-2 rounded-xl border border-slate-200 py-5 text-xs text-slate-500"
        >
          <img :src="settlementIcon" alt="" class="h-[22px] w-[22px]" />
          정산
        </button>
      </div>
    </template>
...
import expenseIcon from '@/assets/icons/workation/expense.svg';
import settlementIcon from '@/assets/icons/workation/settlement.svg';
...
const goBudgetDetail = (budgetType) => {
  console.log('예산 세부내역', budgetType);
};

const goUncheckedExpenses = () => {
  console.log('확인 필요 지출');
};

const goRecord = (workationId) => {
  console.log('정산기록', workationId);
};
</script>
