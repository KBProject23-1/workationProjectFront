...
// TODO: PIN 페이지 완성되면 payload 에 pinNumber/deviceId 받아서 refund 에 전달
async function handleAmountNext(payload) {
  if (isSubmitting.value) return; // 더블클릭/재진입 방어
  isSubmitting.value = true;
  // 같은 시도(금액·계좌 동일)의 재시도면 같은 멱등키 재사용 → 응답 유실 후 재시도 시 중복 환불 방지
  ...
  try {
    ...
    const [data] = await Promise.all([
      walletStore.refund(payload.accountId, payload.amount, '123456', idempotencyKey), // 임시 고정 PIN
