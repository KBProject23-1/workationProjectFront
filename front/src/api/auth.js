...
//
// accessToken/refreshToken 은 HttpOnly 쿠키로만 내려온다(응답 바디에 토큰 없음).
// 응답 바디 data: { userId, name, pinSetupRequired }
...
// 액세스 토큰 재발급
// refreshToken 은 HttpOnly 쿠키로만 오가므로 본문이 없고, 성공 시 새 accessToken/refreshToken 도
// Set-Cookie 로만 내려온다(응답 바디에 토큰 없음).
//
...
export const refresh = () => {
  return axios.post(
    ...
    null,
    // refresh 도 POST 라 CSRF 대상 → 쿠키 전송 + XSRF 헤더 필요
    {
